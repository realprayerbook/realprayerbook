...
const Dashboard: React.FC<DashboardProps> = ({ onJournalClick, onCommunityClick, onLibraryClick, onAdminClick, onHomeClick, onLogout, isAdmin, children }) => {
    ...
    <div className="bg-[#1a1625] min-h-screen font-manrope text-slate-100 flex flex-col">
      ...
      <header className="flex items-center justify-between border-b border-white/10 px-8 py-4 bg-white/5 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center gap-8">
          <button onClick={onHomeClick} className="flex items-center gap-4 text-[#f4d125] hover:opacity-80 transition-opacity">
          ...
          <nav className="flex items-center gap-4 lg:gap-6 overflow-x-auto no-scrollbar py-2">
            <button onClick={onHomeClick} className="text-white/70 hover:text-[#f4d125] text-xs lg:text-sm font-medium transition-colors cursor-pointer whitespace-nowrap">Home</button>
